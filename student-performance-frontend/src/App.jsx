  ...
  const handlePredict = async (formData) => {
    ...
    try {
      ...
      setHistory(prev => {
        if (prev.length > 0) {
          const last = prev[0];
          if (JSON.stringify(last.formData) === JSON.stringify(formData)) {
            return prev; // Ignore duplicate consecutive entries
          }
        }
